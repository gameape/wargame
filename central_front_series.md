...
- 表面=非FP面（FPマーカーがそのまま現FP）、裏面=FP面（FPマーカー+1FPが現FP）
- Friction Point（FP）マーカーをユニットの下に
  - 最大FPを超えたユニットは除去
  - 最大FP：連隊、砲兵、ヘリは5、大隊は4、中隊は3
  - 第1プレイヤーフェイズ（でのOP消費完了時に、だと思う）にFPを1増やすときはFP面に裏返す
  - FP増加
    - +1：OP消費完了時
    - +1：攻撃・防御完了時（砲兵/ヘリのみ、単独防御を除く）
    - +?：戦闘結果

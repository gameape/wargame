...
- 色の意味
  ...
  - 射程：黒低弾道、白高弾道
  - 移動力：白足、黒車輪、黄履帯
  - モラル/防御力：白モラル、黄ポイント防御力、黒はBターゲットの防御力で黒い0はB-0、黒い1はB-1
- バージョン4.x以前のものは違う
...
- コマンドフェイズ
  ...
  - 2. ウェイテッドターン増加
  - 3. インプリメントチェック
  - 4. ダイロールチェック（リザーブ、オルタネート）
  - 5. Opシートにユニット追加
  - 6. Opシート作成
  - 7. 大隊/車両モラル削減DR（フルアワーのみ、夜間は偶数時のみ、攻撃ミッション中は不可）
  - 8. 天候DR（フルアワーのみ）
- 航空機&砲兵フェイズ
  - 0. 双方1D、大きい方がイニシアティブ所持かつ1Pを決定、タイはリロール1P完了後に2P
　   - 1Pがソーティをすべて配置した後、望む順番で解決とOWF
  　 - 2Pがソーティをすべて配置した後、望む順番で解決とOWF
  - 2. プリプロッテッドバラージ実行
...
- アクションフェイズ
  ...
  - 1. 自Firedマーカー除去
  - 2. 直前ターンに自迫撃砲/歩兵砲が配置した煙幕/照明弾マーカー除去
  - 3. 増援配置
...
- 車両
  - 1～6ステップ、通常PファイアでPターゲット、モラルチェックなし、車両モラル対象、オーバーラン可、車両インパルス可、特別ルールでキャリー可
- キャリア
  - 2ステップ、車両インパルス不可、オーバーラン可、モラルチェックなし、車両モラル/大隊モラルの影響なし、損失は大隊モラルに影響
...
- ヘクスの中心点の間を結んだ線（8.1a）
- 遮断と非遮断のいずれか（8.1a）
- 煙幕のあるヘクスに進入できるが、通過できない（8.1b）
- 遮断地形のあるヘクスに進入できるし、遮断地形のあるヘクスを退出できるが、通過できない（8.1c）
- 隣接するヘクスへのLOSは常に非遮断（8.1d）
- 自然地形/建物は絵ごと、村/町/市街はヘクスごと（ヘクスサイド含む）、高さはゲームルールで指定がなければ20メートル（8.1h）
- Spot Elevation/Benchmarkは無視（8.1i）
- ビジビリティと等しい距離まで設定可（7.0）

### 高さの決め方（標準）
- ヒルトップ/ボトムは周囲の等高線と同じ高さの平面扱い（8.2a、8.2c）
- ヒルトップでもボトムでもないヘクスの高さ（8.2b）
  - より低い等高線とより高い等高線を結ぶ、そのヘクスの中心点を通る直線を引く
  - その直線を4等分して、ヘクス中心点が最も近い点を判定する
  - 等高線の間隔が20mではない場合、5m単位で分割する

### 高さの決め方（簡易）
- 中心点が等高線に触れているなら、そのヘクスはその等高線と同じ高さ（8.4a）
- 中心点が等高線に触れていないなら、そのヘクスは最も近いより低い等高線と同じ高さ（8.4a）
- ボトムは周囲の等高線より、等高線の間隔一段階分低い高さの平面扱い（8.4b）

### 高さの決め方（等高線を読みなれてる人向け）
- ヘクス中心点ではなくLOS上の点をPOCにする（8.5a）
- ヒルトップの位置と高さはSpot Elevation/Benchmarkで決める、ない場合は周囲の等高線より等高線の間隔の1/2だけ高い、位置はUSアーミーの教本に従って決める（8.5b）
- ボトムの最低点は周囲の等高線より等高線の間隔の1/2だけ低くするのを原則に、等高線を見てより適切な高さにしてもよい

### LOS判定手順（8.3b）
 1. 射撃（視認、観測）ヘクスと目標ヘクスの中心点を直線で結ぶ
 2. 射撃（視認、観測）ヘクスと目標ヘクスの高さを決める
 3. 各PoCの高さを決める
 4. LOSグリッドに射撃ヘクス、POC、目標ヘクスを置き、LOSより高いPOCがあれば遮断
...
- ロケットは必ずスカッター、目標ヘクスに着弾することはない、砲兵修整表判定なし、連続射撃も速射も不可、アタックゾーンのみ影響、隣接ヘクスへの影響なし、額面は2倍済みの火力
...
## 航空機
- ソーティ1個は航空機1機の1ヘクスor１ユニットへの攻撃
- ソーティ解決手順
  - 解決するソーティと種別を宣言、エリアファイアはスポットあり・高弾道扱い
  - 射撃モードで非FIREDで非SUP/PARで目標ヘクスが射程内かつLOSありの対空砲1ユニットは対空射撃可、FIREDマーカーを置き、それに対するFBOWFを解決
  - 目標の6ヘクス以内、LOSあり、射撃モード、非SUP/PARの友軍ユニットによる観測を宣言（任意）
  - Sorty Success Table判定、成功なら攻撃、失敗でもそのソーティは失われる

## Dawn/Dusk/Twilight（26.0）
...
- 砲兵の照明弾は半径5ヘクス、迫撃砲・歩兵砲は半径3ヘクスを照らす
